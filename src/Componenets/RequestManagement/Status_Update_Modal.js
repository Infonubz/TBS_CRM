...
export default function Status_Update_Modal({
  currentid,
  setIsSaveModal,
  setIsVerifyModal,
  verifyData,
  ...
  console.log(inputValue, "comments_Value");
  ...
  const handlechange = async (valueid, valuedata) => {
    if (
      valueid == 5 ||
      (valueid == 6 && showError == true) ||
      (valueid == 4 && showError == true) ||
      inputValue?.length > 0
    ) {
      if (currentid.startsWith("tbs-op")) {
        ...
        try {
          const data = await PartnerStatusChange(
            valueid,
            valuedata,
            currentid,
            inputValue
