...
import { MdLocationOn, MdDateRange, MdOutlineAccessTime } from "react-icons/md";
import {
  TiArrowUnsorted,
  TiArrowSortedUp,
  TiArrowSortedDown,
...
export default function CancelTable({ activePage, setActivePage, deviceid }) {
  ...
  const getCancellationDetails =
    useSelector((state) => state.crm.cancellation_details) || [];
  console.log(getCancellationDetails, "cancellation details dashboard");
  ...
  const handleFareSort = () => {
    ...
    console.log(getCancellationDetails, "sort details");
  ...
  const handleDateSort = () => {
    ...
    if (sortOrderDate === "ascend") {
      sorted.sort(
        (a, b) =>
          new Date(b.cancel_date_time.slice(0, 10)) -
          new Date(a.cancel_date_time.slice(0, 10))
      );
  ...
  // const currentItems =
  //   getCancellationDetails?.length > 0 &&
  //   getCancellationDetails?.slice(indexOfFirstItem, indexOfLastItem);
  // const currentItems =
  //   sortedData.length > 0
  //     ? sortedData?.slice(indexOfFirstItem, indexOfLastItem)
  //     : getCancellationDetails?.length>0 && getCancellationDetails?.slice?.(indexOfFirstItem, indexOfLastItem);

  const currentItems =
    Array.isArray(sortedData) && sortedData.length > 0
      ? sortedData.slice(indexOfFirstItem, indexOfLastItem)
      : Array.isArray(getCancellationDetails)
      ? getCancellationDetails.slice(indexOfFirstItem, indexOfLastItem)
      : [];
  ...
  const columns = [
    ...
    },
    {
      ...
      //   new Date(b.cancel_date_time.slice(0, 10)),
      // sortOrder:
      //   sortedInfo.columnKey === "cancel_date_time" && sortedInfo.order,
      width: "10vw",
      render: (row, rowdta, index) => {
        return (
          <div
            className={`ml-[1.5vw] text-[#1F4B7F] text-[1vw] py-[0.5vw] ${
              row.cancel_date_time === "Invalid date" ? "text-center" : ""
            }`}
          >
            {row.cancel_date_time === "Invalid date"
              ? "-"
              : `${formatDate(
                  row.cancel_date_time.split(",")[0].trim()
                )} (${row.cancel_date_time.split(",")[1].trim()})`}
            {console.log(row.cancel_date_time, "cancelling date")}
          </div>
        );
      },
    },
  ...
  const data =
    ...
    currentItems?.map((item, index) => ({
      key: index + 1,
      s_no: 1,
      email: item.login_user_email,
      mobile: item.login_user_mobile,
      ...
      hidden_details: (
        <div className="grid grid-cols-5">
              ...
              <div className="flex justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-[0.25vw] text-[1vw]">
                    <MdDateRange />
                    <p>
                      {item.depature_date === null
                        ? "-"
                        : `${formatDate(item.depature_date.slice(0, 10))}`}
                    </p>
                  </div>
                  <div className="flex items-center gap-[0.25vw] text-[1vw]">
                    <MdOutlineAccessTime />
                    <p>{formatTime(item.depature_time)}</p>
                  </div>
                    ...
                    <p className="font-bold text-[1vw] text-right">
                      {item.droping_point_name ? (
                        item.droping_point_name.length >= 15 ? (
                          ...
                          >
                            <p>{`${item.droping_point_name.slice(
                              0,
                              15
                            )}...`}</p>
                          </Tooltip>
                  ...
                  <div className="flex items-center gap-[0.25vw] text-[1vw]">
                    <p>
                      {item.arrival_date === null
                        ? "-"
                        : `${formatDate(item.arrival_date.slice(0, 10))}`}
                    </p>
                    <MdDateRange />
                  </div>
                  <div className="flex items-center gap-[0.25vw] text-[1vw]">
          ...
          <div className="col-span-1 flex items-center justify-center py-[0.5vw] text-[0.9vw]">
            <div className="border-2 border-dotted border-zinc-800 rounded-[1vw] px-[1vw] py-[0.5vw] w-[15vw]">
              <div className="flex justify-between text-[#1F487C]">
              ...
              <div className="flex justify-between border-t-[0.1vw] border-zinc-800 ">
                <p className="font-bold">Refund Amount</p>
                <p className="font-bold">
                  {item.total_fare && item.refund_amt === null
                    ? "-"
                    : `₹ ${Number(item.refund_amt).toFixed(2)}`}
                </p>
                {/* <p className="font-bold">₹ {item.total_fare.replace(/(\.\d*?[1-9])0+$/, '$1').replace(/\.0+$/, '')}</p> */}
              </div>
              {/* <div className="flex justify-between">
  ...
  const fetchGetCancellationDetails = async () => {
    try {
      const data = await GetCancellationDetailsById(
        updatedata,
        SetUpdateData,
        operatorData
      );
  ...
  return (
    ...
    >
      ...
      <div className={`${isRowExpanded ? "mb-[10vh]" : ""}`}>
        {getCancellationDetails?.length > 10 && (
          <div className=" mt-[1vw]  flex justify-between items-center ">
            ...
            <div>
              <ReactPaginate
                ...
                totalItemsCount={getCancellationDetails?.length}
