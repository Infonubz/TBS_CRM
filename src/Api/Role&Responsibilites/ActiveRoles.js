...
export const GetRolesData = async (filter, dispatch) => {
  try {
    ...
    console.log(response.data,"getrolesdata")
...
export const handleRolesearch = async (e, dispatch) => {
  try {
    ...
    dispatch({ type: GET_ROLES, payload: response.data });
    console.log(response.data,"getrolesdata 2")
    return response.data[0];
  } catch (error) {
    handleError(error);
    return null;
  ...
  try {
    const searchTerm = e.target.value
      ...
      searchTerm.trim().length === 0
        ? await api.get(`${apiUrl}/search-op-emp-roles/${userId}/$`)
        : await api.get(`${apiUrl}/search-op-emp-roles/${userId}/${e.target.value}`);
    // const response = await api.get(`${apiUrl}/search-op-emp-roles/${userId}/${e.target.value}`);
    dispatch({ type: GET_OP_EMP_COUNT, payload: response.data });
...
export const handleProSearch = async (filter, e, dispatch, setRoleCount) => {
  ...
  const url2 = `${apiUrl}/search-pro-emp-roles/$`;
