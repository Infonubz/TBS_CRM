...
const endpoint = userID?.startsWith("tbs-pro")
    ? `${apiUrl}/pro-emp-personal-details${typeId === "PROEMP101" ? `/${userID}` : ""}`
...
export const GetEmployeeAddressData = async () => {
    try {
        const response = await api.get(`${apiUrl}/pro-emp-registered-address${typeId === "PROEMP101" ? `/${userID}` : ""}`);
        return response.data;
    } catch (error) {
        handleError(error);
        return null;
...
export const GetEmployeeProfessionalData = async () => {
    try {
        const response = await api.get(`${apiUrl}/pro-emp-professional-details${typeId === "PROEMP101" ? `/${userID}` : ""}`);
        return response.data;
    } catch (error) {
        handleError(error);
        return null;
...
export const GetEmployeeDocumentData = async () => {
    try {
        const response = await api.get(`${apiUrl}/pro-emp-professional-documents${typeId === "PROEMP101" ? `/${userID}` : ""}`);
        return response.data;
    } catch (error) {
        handleError(error);
        return null;
...
export const submitEmpPersonalData = async (
    values,
    fileList,
) => {
