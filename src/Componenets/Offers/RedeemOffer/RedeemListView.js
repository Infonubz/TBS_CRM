...
export default function RedeemListView({
  ...
  const columns = [
              ...
              >
                <div className="border-[0.1vw] border-[#1F487C] rounded-[0.5vw]">
                  <div className="border-dashed text-[1vw]  font-bold bg-[#1F4B7F] border-white border-[0.2vw] rounded-[0.5vw] text-white w-[10vw] flex items-center justify-center">
                    {`${
                      row?.code?.charAt(0) ===
                      row?.code?.charAt(0)?.toLowerCase()
                        ? capitalizeFirstLetter(row?.code).slice(0, 8)
                        : row?.code?.slice(0, 8)
                    }...`}
                  </div>
                </div>
              ...
              <div className="border-[0.1vw] border-[#1F487C] rounded-[0.5vw]">
                <div className="border-dashed text-[1vw] font-bold bg-[#1F4B7F] border-white border-[0.2vw] rounded-[0.5vw] text-white w-[10vw] flex items-center justify-center ">
                  {row?.code?.charAt(0) === row?.code?.charAt(0)?.toLowerCase()
                    ? capitalizeFirstLetter(row?.code)
                    : row?.code}
                </div>
              </div>
            )}
    ...
    },
    {
      ...
      render: (row) => {
        return (
          <div className="flex items-center pl-[2.5vw]">
    ...
    },
    {
      ...
      render: (row) => {
        return (
          <div>
            <p className="text-[1vw] text-[#1F487C] flex pl-[2vw]">{`${dayjs(
              row?.start_date
    ...
    },
    {
      ...
      render: (row) => {
        return (
          <div>
            <p className="text-[1vw] text-[#1F487c] pl-[2.75vw]  ">
              {`0/${row.usage}`}
            </p>
          </div>
        );
      ...
      ? [
          {
            ...
            render: (row) => {
              return (
                <>
                  {row?.req_status === "Active" || row?.req_status === "Draft" ? (<div className="flex items-center justify-center">-</div>) : (row.comments ? (
    ...
    <>
      ...
      >
        <>
          <div className="flex flex-col items-center justify-center">
            ...
            <div className="flex gap-x-[1.5vw] mt-[1.6vw]">
              <button
                className="items-center text-[1vw] font-extrabold shadow-md shadow-black text-white  space-x-[0.7vw] px-[0.8vw] w-[7vw] h-[2vw] bg-[#2A99FF] rounded-[0.5vw]"
                // onClick={() => {
      ...
      >
        <DeleteList
          ...
          api={`${apiUrl}/redeem-offers-deals/${promotionid?.tbs_offer_id}`}
          module={"redeemoffer"}
          filter={offerFilter}
        />
      </ModalPopup>
      ...
      >
        ...
        {offerimage === null ? (
          ...
          <img src={`${apiImgUrl}${offerimage}`} className="w-full h-[15vw] rounded-[0.5vw] mt-[1vw]" />
        )}
      </ModalPopup>
    </>
    // </div>
