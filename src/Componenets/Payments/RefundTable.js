...
import { GetRefundDetails } from "../../Api/Payments/Payments";
import {capitalizeFirstLetter} from "../Common/Captilization"

export default function RefundTable({ activePage, setActivePage }) {
  ...
  const getRefundDetails = useSelector((state) => state.crm.refund_details);
  console.log(getRefundDetails, "refund details dashboard");

  const handleFareSort = (pagination, filters, sorter) => {
    // console.log(sorter,"sortersorter")
    // setSortedInfo(sorter);
    // console.log(sortedInfo,"sorted info")
    // const sorted = getRefundDetails.sort((a, b) => b.total_fare - a.total_fare);
    let sorted = [...getRefundDetails];
    ...
    if (sortOrderFare === "ascend") {
      ...
      sorted = [...getRefundDetails]; // Reset to original order
    ...
    console.log(getRefundDetails, "sort details");
  ...
  const handleGoToPage = () => {
    ...
    setSkip(goToSkip);
    setSpinning(true);
    GetRefundDetails(count, goToSkip, dispatch, setSpinning);
  ...
  const currentItems = sortedData.length > 0 ? sortedData : getRefundDetails;
  ...
  const columns = [
...

    {
      ...
      render: (row, rowdta, index) => {
        return (
          <div className="ml-[4vw] text-[#1F4B7F] text-[1vw] py-[0.5vw] font-bold">
            {row.payment_id}
          </div>
        );
      },
      ...
      render: (row, rowdta, index) => {
        return (
          <div className="ml-[5vw] text-[#1F4B7F] text-[1vw] py-[0.5vw]">
            {capitalizeFirstLetter(row.speed_processed)}
          </div>
        );
      },
    },
  ...
  useEffect(() => {
    setSpinning(true);
    GetRefundDetails(10, 0, dispatch, setSpinning);
  }, []);
      ...
      <div className={`${isRowExpanded ? "mb-[10vh]" : ""}`}>
        ...
        <div className=" mt-[0.5vw]  flex justify-between items-center ">
          <div className="text-[#1f4b7f] flex text-[1.1vw] gap-[0.5vw] ">
            ...
            {/* <span>from</span> */}
            {/* <span className="font-bold"> */}
              {/* {getRefundDetails?.length > 0 ? getRefundDetails?.length : 0} */}
              {/* 100 */}
            {/* </span> */}
            {/* <span>data</span> */}
          </div>
          <div className="flex gap-[0.5vw]">
            <div className="flex">
              <FontAwesomeIcon
                ...
                onClick={() => {
                  if (skip >= 10) {
                    ...
                    setSkip(minSkip);
                    setSpinning(true);
                    GetRefundDetails(count, minSkip, dispatch, setSpinning);
                    setActivePage(1);
                    setIsRowExpanded(false);
                    setExpandedRowKey(null);
              ...
              <FontAwesomeIcon
                ...
                onClick={() => {
                  if (skip) {
                    ...
                    setSkip(oldSkip);
                    setSpinning(true);
                    GetRefundDetails(count, oldSkip, dispatch, setSpinning);
              ...
              />
              <p
                className={`flex items-center border-[0.1vw] border-[#1f4b7f] px-[0.75vw] rounded-[0.25vw] bg-[#1f4b7f] text-white font-bold`}
              >
                {activePage}
              </p>
              ...
              <FontAwesomeIcon
                ...
                onClick={() => {
                  if (currentItems.length >= 10) {
                    console.log(currentItems.length,"c length")
                    const newSkip = skip + 10;
                    setSkip(newSkip);
                    setSpinning(true);
                    GetRefundDetails(count, newSkip, dispatch, setSpinning);
                    setActivePage(activePage + 1);
                    setIsRowExpanded(false);
                    setExpandedRowKey(null);
              ...
              <FontAwesomeIcon
                className={`border-[0.1vw] border-[#1f4b7f] py-[0.5vw] px-[1vw] text-[#1f4b7f] bg-white rounded-[0.25vw] cursor-pointer ${
                  activePage >= 3 ? "cursor-not-allowed" : "cursor-pointer"
                }`}
                onClick={() => {
                  if (skip < 30) {
                    const maxSkip = 30;
                    setSkip(maxSkip);
                    setSpinning(true);
                    GetRefundDetails(count, maxSkip, dispatch, setSpinning);
                    setActivePage(10);
                    setIsRowExpanded(false);
                    setExpandedRowKey(null);
