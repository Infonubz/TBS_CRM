...
export const SubmitAdsData = async (
  ...
  try {
    ...
    GetAdsData(dispatch);
    console.log(response, "ad responseresponse");
    return response.data;
  } catch (error) {
    handleError(error);
    return null;
  ...
  try {
    if (e.target.value) {
      ...
      searchTerm.trim().length === 0
        ? typeId == "PRO1001" ? await api.get(`${apiUrl}/ads-search/tbs-pro1001/$`) : await api.get(`${apiUrl}/ads-search/${proempid}/$`)
        : typeId == "PRO1001" ? await api.get(`${apiUrl}/ads-search/tbs-pro1001/${searchTerm}`) : await api.get(`${apiUrl}/ads-search/${proempid}/${searchTerm}`);
...
export const handleMbleAdsearch = async (e, dispatch) => {
  ...
  try {
    if (e.target.value) {
      ...
      const response =
      searchTerm.trim().length === 0
        ? typeId == "PRO1001" ? await api.get(`${apiUrl}/mobads-search/tbs-pro1001/$`) : await api.get(`${apiUrl}/mobads-search/${proempid}/$`)
        : typeId == "PRO1001" ? await api.get(`${apiUrl}/mobads-search/tbs-pro1001/${searchTerm}`) : await api.get(`${apiUrl}/mobads-search/${proempid}/${searchTerm}`);
...
export const ChangeAdsStatus = async(status,status_id,ad_id,dispatch) => {
const payload = {
  ads_status : status_id === 0 ? "Active" : status_id===2 && "Draft", 
  ads_status_id: status_id === 0 ? 2 : status_id === 2 && 0,
}
const url = `${apiUrl}/updateadStatus/${ad_id}`;
const method = "put";
...
export const ChangeMobAdsStatus = async(status,status_id,ad_id,dispatch) => {
  const payload = {
    ads_status : status_id === 0 ? "Active" : status_id===2 && "Draft", 
    ads_status_id: status_id === 0 ? 2 : status_id === 2 && 0,
  }
  const url = `${apiUrl}/updateMobAdsStatus/${ad_id}`;
  const method = "put";
