...
// import NewPowerbi from "./NewPowerBi";
import { GetBookingDetails, GetBookingDetailsById } from "../../Api/Dashboard/Dashboard";
        ...
        return (
          <div className="flex justify-center text-[#1F4B7F] text-[1vw] py-[0.5vw]">
            {row?.destination_name?.length > 9 ? (
              ...
              <div>
                  {row?.destination_name?.slice(0, 9)}
              </div>
            )}
          </div>
        );
          ...
          <div className="pt-[1vw] px-[3vw]">
              <div className="bg-[#1F487C] rounded-lg">
                ...
                <div className="grid grid-cols-4 gap-x-[1.5vw] justify-items-center py-[0.25vw]">
                  ...
                  {item.passenger_details.map((pass)=><p>{pass.Seat_Num}</p>)}
                  </span>
                  <span className="text-white font-normal text-[1vw]">
                  {item.passenger_details.map((pass)=><p>{pass.Passenger_Name}</p>)}
                  </span>
                  <span className="text-white font-normal text-[1vw]">{item.passenger_details.map((pass)=><p>{pass.Age}</p>)}</span>
                  <span className="text-white font-normal text-[1vw]">{item.passenger_details.map((pass)=><p>{pass.GENDER_TYPE}</p>)}</span>
                </div>
              </div>
            </div>
        ...
        <div className="h-[75vh]">
          {spinning ? (
            ...
            <Table
              ...
              expandable={{
                  expandedRowRender:record => <p style={{ margin: 0 }}>{record?.hidden_details}</p>
              }}
