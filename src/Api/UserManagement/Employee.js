  ...
  try {
    if (e.target.value) {
      ...
      searchTerm.trim().length === 0
        ? user?.startsWith("tbs-pro") ? await api.get(
          `${apiUrl}/proemp-search/${user}/$`) : await api.get(
            `${apiUrl}/employee-search/${user}/$`)
        : user?.startsWith("tbs-pro") ? await api.get(
          `${apiUrl}/proemp-search/${user}/${searchTerm}`) : await api.get(
            `${apiUrl}/employee-search/${user}/${searchTerm}`)
      dispatch({ type: GET_EMPLOYEE_LIST, payload: response.data });
...
export const GetEmpProffesionalById = async (
  ...
  try {
    const response = await api.get(endpoint);
    console.log(response, "getempproff response");
    // SetUpdateData(null);
    setEmpProffesionalData("");
    return response?.data;
  } catch (error) {
