...
export default function CreateActiveRole({
  ...
  const validationSchema = Yup.object().shape({
    ...
    .min(3, "Role Type must be greater than 3 characters")
    // .max(25, "Role Type should not be greater than 25 characters")
    .test(
      'max-25-chars-no-spaces',
      'Role Type should not be greater than 25 characters',
      function(value) {
        const valueWithoutSpaces = value ? value.replace(/\s+/g, '') : '';
        return valueWithoutSpaces.length <= 25;
      }
    )
    ...
    .test(
      ...
      function(value) {
        const valueWithoutSpaces = value ? value.replace(/\s+/g, '') : '';
        return valueWithoutSpaces.length <= 50;
      }
    )
