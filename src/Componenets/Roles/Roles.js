...
export default function Roles() {
  ...
  const currentItems =
    ...
    // const currentItems = getrolelist?.length > 0 ? getrolelist.slice(indexOfFirstItem, indexOfLastItem) : [];
  // Handle page change
  ...
  const updatedItems = Array.isArray(currentItems) && currentItems.map(item => {
    const matchingRole = Array.isArray(roleCount) && roleCount.find(role => role.role === item.role_type);
    return {
      ...item,
      role_count: matchingRole ? matchingRole.role_count : 0, //Default to 0 if no match found
    };
  });
  console.log(roleCount, "role couuuuuunt")
  console.log(currentItems, "role couuuuuunt")
  return (
    <>
      <div
