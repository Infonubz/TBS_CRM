...
import { MdAutoDelete, MdDeleteForever, MdSettingsBackupRestore } from "react-icons/md";
import { useDispatch } from "react-redux";
import { DeleteBinData } from "../../Api/RecycleBin/RecycleBin";
import { toast } from "react-toastify";
const BinDelete = ({setDeleteModalOpen, deleteid,url,title,getId}) =>{


    const dispatch = useDispatch()
    
    const handlesubmit = async() =>{
       const response = await DeleteBinData(dispatch,url,getId)
