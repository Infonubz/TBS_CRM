...
export default function ListView({
  ...
  const handleStatus = (row) => {
    ...
    if (
      ...
      (type_Id === "OP101" &&
        listType === "employee" &&
        !(
          [2, 3, 4].includes(row.promo_status_id) &&
          row.promo_status_id === row.user_status_id
        ))
    ) {
      setStatusUpdateModal(true);
      setPromotionId(row.promo_id);
  ...
  const columns = [
    ...
    {
      title: (
        <h1 className="text-[1.1vw] font-bold flex items-center justify-center sortable-column-title">
          Operator Name
        </h1>
      ),
        ...
        return (
          <div className="flex pl-[1vw] font-bold text-[#1F487C] text-[1vw]">
            {row?.operator_details?.length > 15 ? (
              ...
              >
                {`${
                  row?.operator_details?.charAt(0) ===
                  row?.operator_details?.charAt(0).toLowerCase()
                    ? capitalizeFirstLetterOfEachWord(row?.operator_details?.slice(0, 15))
          ...
          <div className="flex pl-[1.5vw] font-bold text-[#1F487C] text-[1vw]">
            {row?.promo_name?.length > 15 ? (
              ...
              >
                ...
                {`${
                  row?.promo_name?.charAt(0) ===
                  row?.promo_name?.charAt(0).toLowerCase()
      ...
      ? [
          {
            ...
            render: (row) => {
              return (
                <div className="flex items-center justify-center text-[#1F487C]">
                  {row?.promo_status === "Active" || row?.promo_status === "Draft" ? (
                    <span className="text-[#1F487C] text-[1.5vw]">-</span>
                    ...
                    >
                      <span className="text-[#1F487C] text-[1vw]">{`${capitalizeFirstLetter(
        ...
        return (
          <div className="flex items-center justify-center">
            {row?.promo_status_id != null ? (
              <button
                className={`${
                  row.promo_status_id === 0
                    ...
                    ? "bg-[#FF9900]"
                    : row.promo_status_id === 6 && type_Id === "PRO101"
                    ? "bg-[#FF9900]"
                    : row.promo_status_id === 6 && type_Id === "OPEMP101"
                    ? "bg-[#FF9900]"
                    : "bg-[#FF9900]"
              ...
              >
                {type_Id === "OPEMP101" && row.promo_status === "Pending"
                  ? "Posted"
                  : type_Id === "OPEMP101" && row.promo_status === "Repost"
                  ? "Posted"
                  : type_Id === "PRO101" && row.promo_status === "Repost"
                  ? "Pending"
                  : type_Id === "PRO101" && row.promo_status === "Posted"
                  ? "Pending"
        ...
        return (
          <div className="flex px-[1.5vw] items-center justify-between">
            ...
            {type_Id === "PRO101" ? (
              ...
              [2, 3, 4].includes(row.promo_status_id) &&
              row.promo_status_id === row.user_status_id ? null : type_Id ===
                "OPEMP101" &&
              [2, 5, 3, 4, 6].includes(row.promo_status_id) ? null : (
              <MdEdit
                size={"1.3vw"}
                color="#1F4B7F"
                className="cursor-pointer"
                onClick={() => {
                  console.log(
                    type_Id === "OPEMP101" &&
                      [2, 5].includes(row.promo_status_id),
                    "conditionconditionnnnnn"
                  );
                  setPromotionId(row.promo_id);
                  SetUpdateData(row.promo_id);
