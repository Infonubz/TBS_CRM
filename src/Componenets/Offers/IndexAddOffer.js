...
export default function IndexAddOffer({
  ...
  const validationSchema = Yup.object()
    .shape({
      ...
      offer_desc: Yup.string()
        ...
        .max(100, "Max 100 characters only"),
      ...
      file: Yup.mixed()
        ...
        .test("file_size", "File size should be below 5 MB", function (value) {
  ...
  const handleSubmit = async (values) => {
    ...
    if (offerType === "discount") {
      if (values.value_symbol === "₹") {
        if (values.value < 1 || values.value > 2000) {
          setError("Offer value should be between ₹ 1 and ₹ 2000.");
          return;
        }
      } else if (values.value_symbol === "%") {
        if (values.value < 1 || values.value > 99) {
          setError("Offer value should be between 1 % and 99 %.");
          return;
        }
      }
    }
    ...
    if (CurrentPage === 1) {
      ...
      if (!isImageConverted) {
        toast.warning("Please wait and try again (Image is still processing).");
        setLoading(false);
        return;
      }
